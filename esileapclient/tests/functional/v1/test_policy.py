...
class PolicyTests(ESIBaseTestClass):
    ...
    def test_create_lessee(self):
        """ Tests that a lessee cannot create an offer or a lease.
            Test steps:
            ...
            4) Attempt to create a lease as a lessee (using lease create).
    ...
    def test_offer_delete_lessee(self):
        ...
        e = self.assertRaises(CommandFailed,
                              ...
                              self.clients['unauthorized-lessee'],
                              offer['uuid'])
        self.assertIn('Access was denied to esi_leap:offer:delete.',
                      e.stderr.decode())

    @decorators.attr(type=['negative'])
    def test_create_unauthorized(self):
        """ Tests that an owner can't create an offer or a lease for a node
    ...
    def test_offer_claim_unauthorized(self):
        """ Tests that a lessee cannot delete an offer.
            Test steps:
            1) (authorized-owner) Create an offer for an owned node without
                specifying a lessee.
            2) Check that lease details were returned.
